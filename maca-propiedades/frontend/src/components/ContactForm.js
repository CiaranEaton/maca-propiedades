...
const ContactForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    window.open(`https://wa.me/56954327446?text=${encodeURIComponent(text)}`, '_blank');
          ...
          <div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm text-slate-600">Escríbenos al WhatsApp</p>
                  <p className="text-xl font-bold text-[#1a5f7a]">+56 954 327 446</p>
                  <p className="text-xl font-bold text-[#1a5f7a]">+56 945 358 545</p>
                </div>
              </div>
            </div>
          </div>
          <div className="bg-[#f8fafc] rounded-3xl p-8 md:p-10 border border-slate-100">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div><label className="block text-sm font-medium text-slate-700 mb-2">Teléfono *</label>
                <div className="relative"><Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} /><input type="tel" name="phone" value={formData.phone} onChange={handleChange} required className={`${inputClass} pl-12`} placeholder="+56 9 1234 5678" /></div>
              </div>
