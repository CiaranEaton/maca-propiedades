...
const PropertyCard = ({ property, onEdit, onDelete, isAdmin }) => {
  const images = property.image_urls?.length ? property.image_urls : (property.image_url ? [property.image_url] : []);
  const [imgIndex, setImgIndex] = useState(0);

  const currencySymbol = CURRENCY_SYMBOLS[property.currency] || '$';
  const priceDisplay = property.currency === 'CLP'
  ...
  return (
    <div className="property-card group relative bg-white rounded-3xl overflow-hidden border border-slate-100 hover:border-[#00bcd4]/30 transition-all duration-500 shadow-md hover:shadow-xl">
      <div className="aspect-[4/3] overflow-hidden relative bg-slate-100">
        {images.length > 0 ? (
          <img src={images[imgIndex]} alt={property.title} className="property-card-image w-full h-full object-cover" />
        ) : (
        ...
        {images.length > 1 && (
          <>
            <button type="button" onClick={() => setImgIndex(i => Math.max(0, i - 1))} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white p-1.5 rounded-full transition-all">
              <ChevronLeft size={18} />
            </button>
            <button type="button" onClick={() => setImgIndex(i => Math.min(images.length - 1, i + 1))} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white p-1.5 rounded-full transition-all">
              <ChevronRight size={18} />
            </button>
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1">
              {images.map((_, i) => (
                <button key={i} type="button" onClick={() => setImgIndex(i)} className={`h-1.5 rounded-full transition-all ${i === imgIndex ? 'bg-white w-5' : 'bg-white/50 w-2'}`} />
              ))}
            </div>
            <div className="absolute bottom-3 right-3 bg-black/40 text-white text-xs px-2 py-0.5 rounded-full">{imgIndex + 1}/{images.length}</div>
          </>
        )}
      </div>

      <div className="p-6">
        <h3 className="text-xl font-semibold text-[#1a5f7a] mb-2">{property.title}</h3>
        ...
        <p className="text-2xl font-bold text-[#1a5f7a]">{priceDisplay}</p>
      </div>
    </div>
  );
};
