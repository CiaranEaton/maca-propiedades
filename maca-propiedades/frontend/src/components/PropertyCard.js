    ...
    <div className="property-card group relative bg-white rounded-3xl overflow-hidden border border-slate-100 hover:border-[#00bcd4]/30 transition-all duration-500 shadow-md hover:shadow-xl">
      <div className="aspect-[4/3] overflow-hidden relative">
        ...
        <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => onEdit(property)} className="bg-white/90 hover:bg-white p-2 rounded-full transition-all"><Edit className="text-[#1a5f7a]" size={18} /></button>
          <button onClick={() => onDelete(property.id)} className="bg-white/90 hover:bg-white p-2 rounded-full transition-all"><Trash2 className="text-red-500" size={18} /></button>
        </div>
      </div>
      <div className="p-6">
